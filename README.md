...
or test time and are not committed to the repository (all are listed in
`.gitignore`):

| Path | Generated by | When |
| ---- | ------------ | ---- |
| `.next/` | `next build` / `next dev` | Build & dev server |
| `out/` | `next build` (static export) | Production build |
...
| `tests/**/__snapshots__/` | `bun test` | First snapshot test run |
...
| `bun.lock` | `bun install` | Bun lockfile (committed) |

> **Note:** Run `make clean` to remove `.next/`, `out/`, `node_modules/`, and
> `.venv/` in one step. Snapshot files under `tests/**/__snapshots__/` should
> be committed once generated — they are the baseline for regression detection.
